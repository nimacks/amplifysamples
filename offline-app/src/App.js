...
function App() {
  ...
  // Do an initial call to fetch messages
  useEffect(() => {
    fetchMessages();
  ...
  // The onChangeFunction will evaluate the raised event
  // If the event has a hex property we'll know that the event was raised by the color picker and we can then set the color property
  // Otherwise we set the title property
  function onChange(e) {
    if (e.hex) {
  ...
  async function createMessage() {
    ...
    await DataStore.save(new Message({ ...formState })); // save the new message
    updateFormState(initialState); // This will clear the form after a message has been saved
  }
